...
Status: **spike landed** (Settle op + undo). Tuned so typical soft
clay actually moves; not FEM.

## Goal

Give clay a controllable “gradual setting” under its own weight:
tall thin forms slump and puddle outward; soft plasticity also
settles squat / spherical forms. Slider `plasticity ∈ [0, 1]` =
elastic (noop) → soft yielding.
...
3. A soft sphere (`plasticity = 1`) loses height and relocates voxels
   (does not silently no-op).
4. A stiff fat blob (`plasticity ≈ 0.25`) barely moves.
5. Volume of solid voxels stays within ~15% of the pre-settle count
   (approximate conservation — good enough for a spike).
6. Narrow-band meshing does not crumple (band rewritten with the
...
- Stable height is the tighter of:
  - footprint-relative: `footprint × (2.2 − 1.65 × plasticity)`
  - absolute: `5 + (1 − plasticity) × 34` voxels
- A column **yields** when `height > max_stable` (floored at 3).

Excess solid is relocated into the shortest neighbouring columns:
plant just above each neighbour’s own top (empty neighbours plant at
the yielding column’s foot so material flares at the base).

## Yield response (one iteration)

For each yielding column:

1. **Peel** up to `1 + 4×plasticity` solid voxels from the top
   (`φ → +voxel_size`).
2. **Plant** the same count into the shortest neighbouring columns
   (`φ → −voxel_size`), flaring the footprint.
...
| Control | Plasticity slider on a small dialog (default from Preferences) |
...
- `Edit → Preferences…` — default plasticity
